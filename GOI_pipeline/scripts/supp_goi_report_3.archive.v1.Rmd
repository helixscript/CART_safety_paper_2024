...
cond_uniq_sites <- cond_uniq_sites[cond_uniq_sites$patient %in% ALL_pats]
...
gene_stats <- read.csv(
    file.path(outputDir, "cart19_all_gene_stats.csv")) %>%
...
gene_impact <- readRDS(
    file.path(outputDir, "cart19_all_gene_impact.rds")) %>%
  dplyr::select(
...
df6.1 <- dplyr::filter(df6, count >= 2) %>%

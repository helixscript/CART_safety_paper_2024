...
if(
  ...
  o <- readr::read_tsv('~/external_data/ucsc_hg38/cpgIslandExt.txt.gz', col_names = FALSE)
  names(o) <- c('bin', 'chrom', 'chromStart', 'chromEnd', 'name', 'length', 'cpgNum', 'gcNum', 'perCpg', 'perGc', 'obsExp')
  
  CpG_islands <- dplyr::filter(o, chrom %in% paste0("chr", c(1:22, "X", "Y", "M"))) %>%
    mutate(strand = '*') %>%
  ...
  # mcols(DNaseI) <- DNaseI_data
   ...
   o <- readr::read_tsv('~/external_data/ucsc_hg38/wgEncodeRegDnaseClustered.txt.gz', col_names = FALSE)
   names(o) <- c('bin', 'chrom', 'chromStart', 'chromEnd', 'name', 'score', 'sourceCount', 'sourceIds', 'sourceScores')

   DNaseI <- dplyr::filter(o, chrom %in% paste0("chr", c(1:22, "X", "Y", "M"))) %>%
             mutate(strand = '*') %>%
             GenomicRanges::makeGRangesFromDataFrame(keep.extra.columns = TRUE, 
                                                     seqnames.field = 'chrom', 
                                                     start.field = 'chromStart',
...
cond_uniq_list.save <- cond_uniq_list
#cond_uniq_list <- GRangesList(cond_uniq_list[[11]])
...
tTest <- lapply(
      ...
      function(gr){
        ...
        gr %>%
          ...
          getFeatureCounts(refGenes, "refSeq_counts", width = window_size_refSeq) %>%
...
# save.image('~/myImg1.RData')

# 
# test_1 <- cond_uniq_list[1]
# library(magrittr)
# library(GenomicRanges)
# 
# test_1 %>%
...
# tTest2 <- unname(unlist(GRangesList(tTest)))
# 

...
if(
  ...
  gene_tdn_sites <- GRangesList(lapply(split(gene_ranges, 1:length(gene_ranges)), function(x){
    tdn_gr[subjectHits(findOverlaps(x, tdn_gr, ignore.strand = TRUE))]
  }))
 ...
 # }))
  ...
  gene_tp_sites <- GRangesList(lapply(split(gene_ranges, 1:length(gene_ranges)), function(x){
     tp_gr[subjectHits(findOverlaps(x, tp_gr, ignore.strand = TRUE))]
   }))

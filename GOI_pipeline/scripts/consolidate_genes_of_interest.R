...
if(
  ...
  gene_tdn_sites <- GRangesList(lapply(split(consol_gene_ranges, 1:length(consol_gene_ranges)), function(x){
    tdn_gr[subjectHits(findOverlaps(
      x, tdn_gr, maxgap = 5000, ignore.strand = TRUE))]
  }))
  ...
  gene_tp_sites <- GRangesList(lapply(split(consol_gene_ranges, 1:length(consol_gene_ranges)), function(x){

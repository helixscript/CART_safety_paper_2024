...
title: |
    ...
    | CLL Patients only (CR/PRtd & PR/NR Response Groups)
output: 
  pdf_document:
...
cond_uniq_sites <- cond_uniq_sites[cond_uniq_sites$patient %in% CLL_pats]
...
gene_stats <- read.csv(
    file.path(outputDir, "cart19_cll_gene_stats.csv")
  ) %>%
...
gene_impact <- readRDS(
    file.path(outputDir, "cart19_cll_gene_impact.rds")
  ) %>%
  dplyr::select(
...
unlink(list.files(
  path = outputDir, pattern = "_cart19_cll_goi_data.csv", full.names = TRUE
))

write.csv(
  output_impact, 
  file = file.path(
    outputDir, paste0(gsub("-", "", Sys.Date()), "_cart19_cll_goi_data.csv")), 
  quote = TRUE, 
  row.names = FALSE
)

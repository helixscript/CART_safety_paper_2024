...
cart19_clusters <- unlist(GRangesList(lapply(
  ...
  function(i){
    ...
    if(length(cluster_group) > 0) cluster_group$clus.origin <- clus_name
    if(length(cluster_group) > 0) cluster_group$target.min <- clusters[[i]]$target.min
    cluster_group
  }
)))

saveRDS(cart19_clusters,file =file.path(workingDir, 'condensed_intsites', paste0(TRIAL,'_',RESP,'_cart19_clusters.rds')))
